...
export const getPrizes = () =>
  clientApi.get<{ status: boolean; data: PrizeApi[] }>('/prizes');

export const getPrize = (id: number) =>
  clientApi.get<{ status: boolean; data: PrizeApi }>(`/prizes/${id}`);

export const createPrize = (data: CreatePrizeInput) =>
  clientApi.post<{ status: boolean; data: PrizeApi }>('/prizes', data);

export const updatePrize = (id: number, data: UpdatePrizeInput) =>
  clientApi.put<{ status: boolean; data: PrizeApi }>(`/prizes/${id}`, data);

export const deletePrize = (id: number) =>
  clientApi.delete<{ status: boolean; message: string }>(`/prizes/${id}`);

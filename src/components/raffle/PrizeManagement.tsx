...
export function PrizeManagement({
  ...
  const handleAddPrize = () => {
    console.log('Adding prize:', newPrizeName, selectedCategory);
    if (!newPrizeName.trim() || !selectedCategory) return;
    onAddPrize(newPrizeName.trim(), selectedCategory);
    setNewPrizeName('');
    toast.success('Prize added');
  ...
  const getCategoryCount = (category: Category) => {
    const categoryPrizes = getPrizesByCategory(category);
    return {
      total: categoryPrizes.length,
      available: categoryPrizes.filter((p) => !p.isAssigned).length,
    };
  };
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full">
          ...
          {/* Add Button */}
          <Button onClick={handleAddPrize} size="icon" className="self-stretch flex-shrink-0">
            <Plus className="h-4 w-4" />
          </Button>
        </div>
        ...
        <Tabs defaultValue={categories[0]} className="w-full">
                    ...
                    getPrizesByCategory(cat).map((prize) => (
                      <div
                        ...
                        className={`flex flex-col sm:flex-row sm:items-center justify-between p-2 rounded-lg border ${
                          prize.isAssigned ? 'bg-muted/50 opacity-60' : 'bg-card'
                        } gap-2`}
                      >
                        {editingId === prize.id ? (
                          <div className="flex flex-col sm:flex-row items-center gap-2 flex-1 w-full">
                            ...
                            <Button
                              ...
                              className="h-8 w-8"
                              onClick={() => {
                                onUpdatePrize(editingId, editName.trim(), editCategory);
                                setEditingId(null);
                              }}
                            >
                              ...
                              <div className="flex gap-1 mt-1 sm:mt-0">
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    setEditCategory(prize.category);
                                  }}
                                >
          ...
          {/* Drawer */}
          <div
            className={`fixed top-0 right-0 z-50 h-full w-full sm:w-80 bg-white shadow-xl transform transition-transform duration-300 ${
              isCategoryDrawerOpen ? 'translate-x-0' : 'translate-x-full'
            }`}
          >
            <div className="p-4 border-b flex items-center justify-between">
              <h2 className="font-semibold text-lg">Manage Categories</h2>
              <Button size="icon" variant="ghost" onClick={() => setIsCategoryDrawerOpen(false)}>
                <X />
              </Button>
            </div>
            <div className="p-4 space-y-4 overflow-y-auto">
